...
public class GraphicOverlay extends View {
    ...
    private final List<TextGraphic> graphics = new ArrayList<>();
     ...
     * instances to the overlay using {@link GraphicOverlay#(Graphic)}.
     */
     ...
     */
    public void add(TextGraphic graphic) {
        synchronized (lock) {
...
//        listener.onClick(event.getX(),event.getY(),getImageWidth(),getImageWidth());
//        return super.onTouchEvent(event);
//    }
//
//    public interface OnClickListener {
//        void onClick(float x, float y, int width, int height);
//    }
//
...
//    }
     ...
     */
    public TextGraphic getGraphicAtLocation(float rawX, float rawY) {
        synchronized (lock) {
            int[] location = new int[2];
            this.getLocationOnScreen(location);
            for (TextGraphic graphic : graphics) {
